...
// Redis 全局 Redis,使用 db1
var Redis *RedisClient

// ConnectRedis 连接 redis数据库，设置全局的redis对象
...
// Has 判断一个key是否存在，内部错误和redis.Nil 都返回 false
...
// Del 删除存储在redis 里的数据，支持多个key 传参
...
// Increment 当参数只有1个时，为key,其值增加1
// 当参数有2个时，第一个参数为key，第二个参数为要增加的值 int64类型
func (rds RedisClient) Increment(parameters ...interface{}) bool  {
	switch len(parameters) {
	case 1:
		key :=parameters[0].(string) // 断言
